...
module.exports = class DatabaseHandler {
     ...
     */
    async updateGuildAPIToken(guildID, token, discordID, createdAt = new Date()) {
     ...
     */
    async revokeJWT(token, revokedByDiscordID, revokedAt = new Date()) {
    ...
    async createGuildSubscription(guildID, { expiresAt = new Date(), createdAt = new Date(), subLabel, guildsCount = 1, patreonUserID }) {
        ...
        await this.postgres.query(`
            ...
            ($1, $2);
        `, subID, guildID);
        const guildSubscriptions = await this.redis.getString(`guild_subscriptions_${guildID}`, true);
    ...
    async updateGuildSubscription(subID, guildID, settingName, newSettingValue) {
        ...
        const redisUpdatePromise = this.redis.getString(`guild_subscriptions_${guildID}`, true).then((guildSubscriptions) => {
            if (guildSubscriptions) {
                ...
                const newSubscription = { ...guildSubscription, ...{ [settingName]: newSettingValue } };
                const newGuildSubscriptions = [
    ...
    async updateGuildSetting(guildID, settingName, newSettingValue) {
        if (!["language", "prefix", "cmd_channel", "fake_threshold", "storage_id"].includes(change_case_1.snakeCase(settingName)))
            throw new Error("unknown_guild_setting");
        const redisUpdatePromise = this.redis.setHash(`guild_${guildID}`, {
